...
function FileUploader(props: FileUploaderType) {
	...
	const getHeaderTitle = () => {
		if (isPaused) {
			const remainingFiles = Object.values(files).filter(
				f => f && f.status !== 'completed'
			).length;
			return `Paused - ${remainingFiles} files remaining`;
		}
		...
		if (isUploading) {
			...
			return `Uploading ${uploadingFiles} files`;
		}

		if (allFilesCompleted) {
			return `Uploaded ${Object.values(files).length} images`;
		}

		return 'Upload Images';
	};

	// Helper function to calculate total time left
	const getTotalTimeLeft = () => {
		return Object.values(files)
			.filter(f => f && f.status !== 'completed')
			.reduce((sum, f) => sum + (f?.timeLeft || 0), 0);
	};

	// Determine when to show action buttons
	const showActionButtons = isUploading || isPaused || allFilesCompleted;

	// Determine when to show background progress bar
	const showBackgroundProgress =
		isMinimized && (isUploading || isPaused || allFilesCompleted);

	// Determine when to show WhipProgress
	const showWhipProgress =
		isMinimized && (isUploading || isPaused || allFilesCompleted);

	// Determine when to show individual file progress
	const showFileProgress =
		!isMinimized && (isUploading || isPaused || allFilesCompleted);

	// Determine when to show grid
	const showGrid = !isUploading && !isPaused && !allFilesCompleted;

	return (
		<Dialog onOpenChange={handleResetUploader}>
			...
			>
				<div className=" w-full relative">
					{showBackgroundProgress && (
						<motion.div
							className="bg-gray-0 -z-10 absolute inset-0 h-full w-[30%] transition-transform "
							initial={{ width: 0 }}
					...
					{/* Whip progess Section - Only show when minimized */}
						...
						{showWhipProgress && (
							<div className=" space-y-1 mb-4">
								<WhipProgress
									height={25}
									progress={progress}
									initialMode={isPaused ? 'paused' : 'playing'}
									ref={whipRef}
								/>
							</div>
						)}
					</AnimatePresence>
					<div className={`${!isMinimized && 'p-6'} `}>
										...
										return (
											<div key={slot} className="space-y-2">
												<div className="flex justify-between text-sm">
													<span className="font-medium text-gray-500 truncate max-w-[200px] font-inter">
														{file.file.name}
													</span>
						...
						{/* Start Upload Button - Shows when pending OR Continue when completed */}
						{showUploadButton && progress <= 0 && (
							<motion.button
								initial={{ opacity: 0, y: 10 }}
								animate={{ opacity: 1, y: 0 }}
						...
						{showGrid && (
							<motion.p
								initial={{ opacity: 0 }}
								animate={{ opacity: 1 }}
								className="text-xs text-gray-400 text-center leading-relaxed px-5 mt-3"
							>
